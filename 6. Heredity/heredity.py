...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    # print(f"Probabilities in update() function: {probabilities}")

    for person in probabilities:
        if person in one_gene:
            probabilities[person]['gene'][1] = p
        elif person in two_genes:
            probabilities[person]['trait'][2] = p
        else:
...
def normalize(probabilities):
    ...
    """
    print_dictionary(probabilities)
...
def print_dictionary(data):
    ...
    for person, groups in data.items():
        flat_dict = {}
        for group_name, subdict in groups.items():
            for key, value in subdict.items():
                # Create a tuple key (group, specific key)
                flat_dict[(group_name, key)] = value
        rows.append(flat_dict)
        names.append(person)

    # Create DataFrame using the collected data and assign the person names as the index
    df = pd.DataFrame(rows, index=names)

    # Optionally, sort the columns for a cleaner look
